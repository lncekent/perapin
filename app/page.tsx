...
export default function PeraPinApp() {
  ...
  const [role, setRole] = useState<"landing" | "merchant" | "customer">(
    "landing",
  );
  ...
  const [merchantStep, setMerchantStep] = useState<
    "scan" | "confirm" | "pin" | "processing" | "result"
  >("scan");
  const [scannedCustomerId, setScannedCustomerId] = useState<string>("");
  const [scannedCustomerData, setScannedCustomerData] =
  ...
  const [customerStep, setCustomerStep] = useState<
    "dashboard" | "signup" | "pin_creation" | "creating_wallet" | "qr_display"
  >("signup");
    ...
    try {
      ...
      if (videoRef.current) {
        ...
        logToInspector(
          ...
          "Opened device back camera feed for real-time QR scanning.",
        );
      }
    } catch (err) {
      ...
      logToInspector(
        ...
        "Permissions denied or camera is unavailable. Reverting to interactive simulator.",
      );
    }
  }, [logToInspector]);
  ...
  useEffect(() => {
    window.requestAnimationFrame(() => {
      ...
      logToInspector(
        "info",
        "System Initialized",
        "PeraPin cryptographic ledger and state engine loaded from localStorage.",
      );
    });
  }, [logToInspector]);
  ...
  const enterFlow = (selectedRole: "merchant" | "customer") => {
    ...
    logToInspector(
      ...
      `Navigated to ${selectedRole} workspace.`,
    );
  ...
  const handleSimulatedScan = (customerId: string) => {
    ...
    if (!customer) {
      logToInspector(
        "warning",
        "Scan Rejected",
        `Customer ID ${customerId} not found in localized blockchain state.`,
      );
    ...
    logToInspector(
      "success",
      "QR Code Scanned",
      `Successfully scanned static QR sticker for ${customer.name} (${customer.customerId})`,
      { customerId },
    );
  ...
  const handleAmountSubmit = async (e: React.FormEvent) => {
    ...
    logToInspector(
      "info",
      "Requesting Soroban Nonce",
      `Retrieving one-time session sequence hash for customer ${scannedCustomerData.customerId}`,
    );
    ...
    try {
      ...
      if (data.success) {
        ...
        logToInspector(
          "blockchain",
          "Nonce Allocated",
          `Soroban state storage allocated Nonce [${data.nonce}] for security verification. Expires in 5m.`,
          data,
        );
      } else {
      ...
      // Fallback
      const fallbackNonce = Math.floor(
        100000 + Math.random() * 900000,
      ).toString();
      setCurrentNonce(fallbackNonce);
      logToInspector(
        "warning",
        "Nonce Generated Locally (Offline Fallback)",
        `Failed to connect to API, fallback to local seed nonce: ${fallbackNonce}`,
      );
    }
  };
  ...
  const executeSorobanSettlement = async (submittedPin: string) => {
    ...
    logToInspector(
      "info",
      "Settlement Started",
      "Customer handed phone back. Commencing client-side PIN hashing...",
    );
    ...
    try {
      ...
      clientHash = hashArray
        .map((b) => b.toString(16).padStart(2, "0"))
        .join("");
      logToInspector(
        "blockchain",
        "Client Hash Formed (SHA-256)",
        `Preimage: "${scannedCustomerData.customerId}:****:${currentNonce}:${amountFloat}"\nHash: ${clientHash}`,
      );
    } catch (err) {
      ...
      logToInspector(
        "warning",
        "Hash Generated with Fallback Method",
        `Constructed mock hash: ${clientHash}`,
      );
    }
    ...
    try {
      ...
      if (res.status === 200 && data.success) {
        ...
        const updatedScannedData =
          updatedCustomers.find(
            (c) => c.customerId === scannedCustomerData.customerId,
          ) || null;
      ...
      let errFriendly =
        "Soroban Smart Contract rejected the transaction parameters.";

      if (errCode === "AUTHENTICATION_FAILED") {
        errFriendly =
          "Incorrect 4-digit PIN. The security signature did not match the customer's on-chain key.";
      } else if (errCode === "INSUFFICIENT_FUNDS") {
        errFriendly =
          "Insufficient funds. The customer's on-chain wallet balance is lower than the transaction amount.";
      }
      ...
      logToInspector(
        "warning",
        "Soroban Execution Reverted",
        `Tx Reverted: ${errFriendly}`,
        { errCode },
      );
    }
  };
  ...
  const handleSignupSubmit = async (e: React.FormEvent) => {
    ...
    logToInspector(
      ...
      "KYC stub validated. Proceeding to PeraPin authorization setup.",
    );
  };
  ...
  const createStellarWallet = async () => {
    ...
    logToInspector(
      "info",
      "Deploying Account on Stellar",
      "Creating keypair, allocating initial simulated PHP balance, indexing static QR map...",
    );
    ...
    try {
      ...
      if (res.status === 200 && data.success) {
        ...
        const signupBonusTx: Transaction = {
          ...
          txHash:
            "0x" + Math.floor(Math.random() * 1e16).toString(16) + "00000",
        };
      ...
      setSignupError(
        err.message || "Failed to finalize account setup. Please try again.",
      );
      setCustomerStep("signup");
      logToInspector(
        "warning",
        "Wallet Deployment Reverted",
        `Failed to complete sign up: ${err.message}`,
      );
    }
  };
  ...
  useEffect(() => {
    if (
      activeCustomer &&
      (customerStep === "qr_display" || customerStep === "dashboard") &&
      qrCanvasRef.current
    ) {
      QRCode.toCanvas(
        qrCanvasRef.current,
        ...
        (error) => {
          if (error) {
            ...
            logToInspector(
              ...
              "Could not draw static QR sticker canvas.",
            );
          } else {
  ...
  const resetAppLedgers = () => {
    if (
      confirm(
        "Are you sure you want to reset all local transaction ledgers and customer balances?",
      )
    ) {
      ...
      logToInspector(
        "warning",
        "Database Cleared",
        "Reset all ledger parameters and loaded default seed wallets.",
      );
    }
  };

  // Filter out transaction history for active customer
  const filteredCustomerTxs = transactions.filter(
    (tx) =>
      tx.partnerId === "SYSTEM" ||
      tx.partnerId === "PROMO_SEED" ||
      tx.partnerName.includes("PeraPin") ||
      (activeCustomer &&
        (tx.partnerId === activeCustomer.customerId ||
  ...
  return (
    <div className="flex flex-col min-h-screen relative overflow-x-hidden bg-slate-50 font-sans text-slate-900">
      {/* Dynamic Header */}
      <header
        className="sticky top-0 z-40 bg-white/95 backdrop-blur-md border-b border-slate-200 py-4 px-6 shadow-xs"
        id="header"
      >
          ...
          <div className="flex items-center gap-2">
            ...
            <div
              onClick={() => {
                ...
                logToInspector(
                  "warning",
                  "Network Status Shifted",
                  `Switched simulation to ${!isOfflineMode ? "Local Zero-Connectivity (Offline Mode)" : "Cloud API Connection"}`,
                );
              }}
      ...
      <main
        className="flex-1 max-w-md w-full mx-auto py-4 px-6 flex flex-col justify-start relative"
        id="main-content"
      >
          ...
          {role === "landing" && (
            ...
            >
              ...
              <div className="text-center space-y-4 pt-4" id="landing-brand">
                <div
                  className="inline-flex items-center justify-center bg-blue-50 border border-blue-100 text-blue-600 px-3 py-1 rounded-full text-xs font-medium mb-2"
                  id="chain-badge"
                >
                ...
                <p
                  ...
                  id="landing-tagline"
                >
                  Pay and get paid, even offline. No phone, data, or power
              ...
              <div className="space-y-4" id="role-select-buttons">
                ...
                >
                  <div className="flex items-center gap-4 text-left">
                    <div
                      className="bg-white/25 text-white p-3 rounded-xl transition-colors"
                      id="merchant-icon-box"
                    >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-blue-100">
                        Scan stickers & accept payments with your phone browser
                      </p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center gap-4 text-left">
                    <div
                      className="bg-blue-50 text-blue-600 p-3 rounded-xl transition-colors"
                      id="customer-icon-box"
                    >
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500">
                        Generate static payment sticker. No phone needed to buy.
                      </p>
                    </div>
                  </div>
              ...
              >
                ...
                >
                  <div className="space-y-1.5" id="step-1">
                    <div className="w-8 h-8 rounded-full bg-slate-50 text-slate-600 flex items-center justify-center text-xs font-bold mx-auto border border-slate-200">
                      1
                    </div>
                    ...
                    <p className="text-[10px] text-slate-500 leading-normal">
                      Merchant scans consumer QR sticker
                    </p>
                  </div>
                  <div className="space-y-1.5" id="step-2">
                    ...
                    <p className="text-[10px] text-slate-500 leading-normal">
                      Customer keys PIN on merchant phone
                    </p>
                  </div>
                  <div className="space-y-1.5" id="step-3">
                    <div className="w-8 h-8 rounded-full bg-blue-50 text-blue-600 flex items-center justify-center text-xs font-bold mx-auto border border-blue-200">
                      3
                    </div>
                    ...
                    <p className="text-[10px] text-slate-500 leading-normal">
                      Hash settles instantly on Soroban
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="text-center" id="landing-footer">
                <p className="text-[11px] text-slate-400">
                  Soroban contract CC_PERAPIN_V1 is simulated in sandboxed local
                  memory.
                </p>
              </div>
            </motion.div>
          ...
          {role === "merchant" && (
            ...
            >
              ...
              >
                ...
                <div className="text-right" id="merchant-selector-container">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const m = DEMO_MERCHANTS.find(
                        (merch) => merch.id === e.target.value,
                      );
                      if (m) {
                        setActiveMerchant(m);
                        logToInspector(
                ...
                >
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-slate-550 leading-normal">
                      Scan the printed QR sticker carried by the customer (on
                      their tricycle card, keychain, or back of phone) to
                      initiate payment.
                    </p>
                  </div>
                  ...
                  <div
                    className="relative aspect-square w-full max-w-[320px] mx-auto bg-slate-100 rounded-3xl overflow-hidden border border-slate-200 shadow-inner flex flex-col items-center justify-center"
                      ...
                      >
                        ...
                        <div className="space-y-1">
                          <p className="text-sm font-bold text-slate-800">
                            Device camera standby
                          </p>
                          <p className="text-[11px] text-slate-500 px-4 leading-normal">
                            You can grant camera access or use the immediate
                            interactive simulator buttons below to test
                            instantly.
                          </p>
                        </div>
                        <button
                  ...
                  <div className="space-y-2" id="simulated-scanners">
                    ...
                    <div className="bg-white border border-slate-200 shadow-xs rounded-2xl p-4 space-y-3">
                      <p className="text-[10px] text-slate-500 text-center leading-tight">
                        Tap any customer wallet below to simulate scanning their
                        static PeraPin QR code:
                      </p>

                      <div
                        className="grid grid-cols-2 gap-2"
                        id="demo-scannable-customers"
                      >
                        {customers.map((c) => (
                          <button
                            key={c.customerId}
                      ...
                      >
                        ...
                        >
                          <input
                            ...
                            onChange={(e) =>
                              setScannedCustomerId(e.target.value)
                            }
              ...
              {merchantStep === "confirm" && scannedCustomerData && (
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div
                      className="bg-white border border-slate-200 shadow-sm rounded-2xl p-4 flex items-center justify-between"
                      id="customer-identity-card"
                    >
                      <div className="space-y-1">
                        <span className="text-[10px] text-slate-400 block uppercase font-mono tracking-wider">
                          Identified Consumer
                        ...
                        <h4 className="font-bold text-base text-slate-800">
                          {scannedCustomerData.name}
                        </h4>
                        <span className="font-mono text-xs text-blue-600">
                          {scannedCustomerData.customerId}
                        </span>
                      </div>
                    ...
                    >
                      <div className="space-y-1">
                        ...
                        <div className="relative flex items-center">
                          <span className="absolute left-4 text-3xl font-extrabold text-slate-400 font-mono">
                            ₱
                          </span>
                          <input
                            id="payment-amount"
                      ...
                      <div className="text-[11px] text-slate-500 leading-relaxed flex items-start gap-1.5 bg-blue-50/50 p-3.5 rounded-xl border border-blue-100">
                        ...
                        <span>
                          This value will be combined client-side with Soroban
                          Nonce [{currentNonce || "pending"}] and the{" "}
                          {"customer's"} PIN to lock in a transaction block.
                        </span>
                      </div>
                    </form>
                  </div>
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-slate-550 max-w-xs mx-auto leading-normal">
                        Customer: please enter your secret 4-digit PIN to
              ...
              {merchantStep === "processing" && (
                <div
                  className="flex-1 flex flex-col justify-center items-center py-12 space-y-6"
                  id="merchant-step-processing"
                >
                  ...
                  <div className="text-center space-y-2">
                    ...
                    <div
                      className="text-xs text-slate-600 space-y-1 pt-2 font-mono max-w-xs mx-auto text-left border border-slate-200 p-4 rounded-xl bg-slate-50 shadow-xs"
                      id="processing-steps-log"
                    >
              ...
              {merchantStep === "result" && recentReceipt && (
                ...
                >
                  <div className="space-y-4">
                    ...
                    <div
                      className="relative bg-white border border-slate-200 rounded-3xl p-5 shadow-sm overflow-hidden"
                      id="receipt-box"
                    >
                      ...
                      <div className="space-y-4 text-xs" id="receipt-details">
                        <div className="flex justify-between items-center pb-3 border-b border-slate-100">
                          <div>
                            ...
                            <span className="text-[10px] text-slate-400 font-mono">
                              ID: {recentReceipt.txId || "F-AUTH-ERR"}
                            </span>
                          </div>
                          <span
                        ...
                        <div
                          ...
                          id="receipt-amount-display"
                        >
                          <span className="text-[10px] uppercase font-mono text-slate-400 block">
                            TOTAL PHP SETTLED
                          </span>
                          <span
                            className={`text-4xl font-extrabold font-mono ${recentReceipt.success ? "text-blue-600" : "text-slate-500"}`}
                          >
                        ...
                        <div
                          className="space-y-2 border-t border-b border-slate-100 py-3 font-mono text-[11px]"
                          id="receipt-table"
                        >
                          ...
                          {recentReceipt.success && (
                            <>
                              <div className="flex justify-between">
                                <span className="text-slate-400">
                                  Stellar Ledger index:
                                </span>
                                <span className="text-emerald-600 text-right">
                                  #{recentReceipt.ledgerSequence}
                                </span>
                              </div>
                              <div className="flex justify-between">
                                <span className="text-slate-400">
                                ...
                                <span className="text-slate-600 text-right">
                                  {recentReceipt.stellarFeeXlm} XLM
                                </span>
                              </div>
                            </>
                          )}
                          ...
                          <div className="p-3 bg-red-50 border border-red-100 text-red-700 rounded-xl flex items-start gap-2">
                            ...
                            <div className="space-y-0.5">
                              <p className="font-bold text-[11px]">
                                Reason: {recentReceipt.error}
                              </p>
                              <p className="text-[10px] text-slate-500 leading-normal">
                                {recentReceipt.errorMessage}
                              </p>
                            </div>
                          </div>
                        )}
            ...
            >
                ...
                >
                  <div className="space-y-4">
                    <div className="space-y-1">
                      ...
                      <p className="text-xs text-slate-500 leading-normal">
                        Create your zero-connectivity payment account. This
                        generates a secure Stellar public/private key
                        combination linked to a static QR sticker.
                      </p>
                    </div>

                    {signupError && (
                      <div
                        className="p-3 bg-red-50 border border-red-100 text-red-700 rounded-xl flex items-start gap-2 text-xs"
                        id="signup-error"
                      >
                        <AlertCircle className="w-4 h-4 flex-shrink-0" />
                    ...
                    >
                      ...
                      <div className="space-y-1">
                        <label
                          className="text-xs font-medium text-slate-500"
                          htmlFor="signup-kyc-type"
                        >
                          KYC ID Document Type
                        </label>
                        <select
                          id="signup-kyc-type"
              ...
              {customerStep === "pin_creation" && (
                ...
                >
                  <div className="space-y-4">
                    ...
                    <p className="text-xs text-slate-500 leading-normal">
                      This 4-digit PIN is your absolute spending credential. It
                      will never be stored or shared on the network. Payments
                      are authorized by combining this PIN with a one-time nonce
                      client-side before hashing.
                    </p>
                    ...
                    <div className="space-y-4 pt-2" id="pin-inputs-grid">
                      <div className="space-y-1">
                        ...
                        <input
                          ...
                          value={createdPin}
                          onChange={(e) =>
                            setCreatedPin(e.target.value.replace(/\D/g, ""))
                          }
                  ...
                  <button
                    ...
                    disabled={
                      createdPin.length !== 4 || confirmPin.length !== 4
                    }
                    className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-slate-200 disabled:text-slate-400 text-white py-3.5 rounded-2xl font-bold text-sm tracking-tight transition-all active:scale-[0.99] min-h-[44px]"
              ...
              {customerStep === "creating_wallet" && (
                ...
                >
                  ...
                  <div className="text-center space-y-2">
                    <span className="text-[10px] font-mono text-blue-600 uppercase tracking-widest block animate-pulse font-bold">
                      Stellar ledger integration
                    </span>
                    <h3 className="text-lg font-bold text-slate-800">
                      Deploying Soroban Identity
                    </h3>

                    {/* Simulated pipeline stages */}
                    <div
                      className="text-xs text-slate-600 space-y-1 pt-2 font-mono max-w-xs mx-auto text-left border border-slate-200 p-4 rounded-xl bg-slate-50 shadow-xs"
                      id="wallet-creation-steps-log"
                    >
                      <div className="flex justify-between items-center text-blue-600">
                ...
                >
                  <div className="space-y-4">
                    <div className="text-center space-y-1">
                      <span className="text-[10px] font-mono text-blue-600 uppercase tracking-widest block font-bold bg-blue-50 border border-blue-100 px-3 py-1 rounded-full text-xs mb-1">
                        Registration Successful
                      </span>
                      <h2 className="text-xl font-bold text-slate-800 tracking-tight">
                        Your Static QR Sticker
                      </h2>
                      <p className="text-xs text-slate-500 max-w-xs mx-auto leading-normal">
                        Print or save this QR code sticker. This represents your
                        wallet address—no phone needed to buy!
                      </p>
                    </div>

                    {/* QR Sticker canvas card */}
                    <div
                      className="bg-white text-slate-900 rounded-3xl p-5 max-w-[280px] w-full mx-auto shadow-md flex flex-col items-center space-y-3 border border-slate-200"
                      id="qr-sticker-card"
                    >
                      ...
                      <div
                        className="bg-slate-50 p-2.5 rounded-2xl relative border border-slate-100"
                      ...
                      <div className="text-center space-y-0.5 w-full">
                        <p className="font-bold text-base tracking-tight text-slate-900">
                          {activeCustomer.name}
                        </p>
                      ...
                      <span className="text-[8px] font-mono text-slate-400 leading-normal text-center">
                        STELLAR ADDR:{" "}
                        {activeCustomer.stellarPublicKey.slice(0, 12)}...
                      </span>
                    </div>
                    ...
                    <div
                      className="p-4 bg-slate-50 border border-slate-200 rounded-2xl text-xs text-slate-600 leading-normal space-y-1.5"
                    ...
                    >
                      ...
                      <ul className="list-disc list-inside space-y-1 pl-1 text-[11px]">
                        <li>
                          Show this printed sticker or paper card to the
                          micro-merchant.
                        </li>
                        <li>
                          They will scan the sticker using their {"phone's"} web
                          browser.
                        </li>
                        <li>
                          Verify the amount, key in your secret 4-digit PIN, and{" "}
                          {"you're"} done!
                        </li>
                      </ul>
                    </div>
              ...
              {customerStep === "dashboard" && activeCustomer && (
                <div
                  ...
                  id="customer-step-dashboard"
                >
                  {/* Quick Card Balance display */}
                  <div
                    className="bg-white border border-slate-200 shadow-xs rounded-3xl p-5 flex flex-col justify-between space-y-4"
                  ...
                  >
                    ...
                    {filteredCustomerTxs.length === 0 ? (
                      ...
                      <div
                        className="space-y-2 overflow-y-auto max-h-[220px]"
                        id="tx-log-scroll"
                      >
