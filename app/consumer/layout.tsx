  ...
  return (
    <div className="min-h-screen bg-slate-50">
          ...
          <Link href="/consumer/dashboard" className="flex items-center gap-2">
            ...
            <div>
              <span className="block font-bold text-slate-900">
                PeraPin{" "}
                <span className="-mt-0.5 block text-xs font-medium text-slate-400">Consumer</span>
              </span>
            </div>
          </Link>
          <button
            onClick={() => setLogoutOpen(true)}
            className="inline-flex min-h-11 items-center gap-1.5 rounded-xl px-3 text-sm font-medium text-slate-500 transition-colors hover:bg-red-50 hover:text-red-600"
          >
      ...
      <ConfirmDialog
        ...
        title="Log out?"
        description="You’ll need to sign in again with an email code to access your wallet."
